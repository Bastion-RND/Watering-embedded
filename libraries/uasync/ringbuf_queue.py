# https://github.com/peterhinch/micropython-async/blob/master/v3/primitives/ringbuf_queue.py
...
# put_nowait QueueFull exception can be ignored allowing the oldest data to be discarded.
...
class RingbufQueue:  # MicroPython optimised
    def __init__(self, buf):
        ...
        self._size = buf if isinstance(buf, int) else len(buf)
        self._wi = 0
        self._ri = 0
    ...
    def put_nowait(self, v):
        ...
        if self._wi == self._ri:  # Would indicate empty
            self._ri = (self._ri + 1) % self._size  # Discard sig_pin message
